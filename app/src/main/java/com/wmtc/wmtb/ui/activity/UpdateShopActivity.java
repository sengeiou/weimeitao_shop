 ...
 */
public class UpdateShopActivity extends CommonPresenterActivity {
    ...
    public void initRootData(View view) {
        ...
        mShop = mBundle.getParcelable("bean");
        if (mShop != null) {
            mTvName.setText(StringUtils.init().fixNullStr(mShop.shopName));
            mTvLocal.setText(StringUtils.init().fixNullStr(mShop.address));
            boolean ls = "已办理好".equals(mShop.licensestatus);
            mRbOk.setChecked(ls);
            mRbNot.setChecked(!ls);
            boolean lt = "企业法人".equals(mShop.licensetype);
            mRbCompany.setChecked(lt);
            mRbOne.setChecked(!lt);
            mEditLicenseId.setText(StringUtils.init().fixNullStr(mShop.licenseid));
            mTvPushLicenseName.setText(StringUtils.init().fixNullStr(mShop.licensename));
            mTvPushLicensePic.setText(StringUtils.isNotBlank(mShop.licensepic) ? "已上传" : "未上传");
            mTvSelectType.setText(StringUtils.init().fixNullStr(mShop.idcardtype));
            mEditName.setText(StringUtils.init().fixNullStr(mShop.idcardname));
            mEditIdNum.setText(StringUtils.init().fixNullStr(mShop.idcard));
            boolean it = "永久有效".equals(mShop.idcardtime);
            mRbTimeAll.setChecked(it);
            mRbTimeEnd.setChecked(!it);
            mTvPushIdPic.setText(StringUtils.isNotBlank(mShop.idcardpic) ? "已上传" : "未上传");
        }
        tvToReSelect.setVisibility(View.INVISIBLE);
    ...
    public void createOk() {
        super.createOk();
        ToastUtils.init().showInfoToast(mActivity, "店铺信息修改成功，请耐心等待审核");
